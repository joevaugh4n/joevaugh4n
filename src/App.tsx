...
function Page({ children, h1, h2, bg = 'accent' }: PageProps) {
  return (
    <section className={`flex justify-center items-center flex-col text-left min-h-[90vh] border-b-1 border-slate-50 px-8 py-[10vh] ${bg == 'primary' ? 'bg-blue-400 text-white' : 'bg-white'}`}>
...
function App() {
  return (
    <div className='md:text-2xl text-xl antialiased'>
      ...
      <Page bg='accent'>
        <section className='flex lg:flex-row flex-col gap-x-12 gap-y-4 border border-slate-50 rounded-lg md:p-32 p-8'>
          <img className='md:w-40 md:h-40 w-1/4 h-1/4 rounded-full lg:self-start self-center' src={joe} alt="Joe Vaughan"/>
          <div>
